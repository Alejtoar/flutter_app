...
class _PlatoEditScreenState extends State<PlatoEditScreen> {
  ...
  void _guardar() async {
    ...
    debugPrint('===> [_guardar] Plato construido: id=${plato.id}, nombre=${plato.nombre}, categorias=${plato.categorias}, receta=${plato.receta}, descripcion=${plato.descripcion}');
    ...
    if (widget.plato == null) {
      ...
      final creado = await controller.crearPlatoConRelaciones(plato, _intermedios, _insumos);
      exito = creado != null;
    } else {
      debugPrint('===> [_guardar] Actualizando plato existente');
      exito = await controller.actualizarPlatoConRelaciones(plato, _intermedios, _insumos);
    }
    if (!mounted) {
      debugPrint('===> [_guardar] Widget desmontado después de guardar, no navego ni muestro SnackBar');
      return;
    }
    if (exito) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(controller.error ?? 'Error al guardar el plato')),
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.plato == null ? 'Crear Plato' : 'Editar Plato')),
      body: Padding(
            ...
            children: [
              if (_codigoGenerado != null)
                Padding(
                  ...
                  child: InputDecorator(
                    ...
                    child: Text(_codigoGenerado!, style: const TextStyle(fontWeight: FontWeight.bold)),
                  ),
                ),
              TextFormField(
              ...
              TextFormField(
                ...
                validator: (v) {
                  ...
                  if (n == null || n < 1) return 'Debe ser un número entero mayor a 0';
                  return null;
                },
              ),
              Padding(
              ...
              Row(
                ...
                children: [
                  const Text('Insumos requeridos', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ElevatedButton.icon(
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Intermedios requeridos', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ElevatedButton.icon(
              ...
              SizedBox(
                ...
                child: ListaIntermediosRequeridos(
                  ...
                  onEliminar: (iu) {
                    setState(() {
                      _intermedios.removeWhere((x) => x.intermedioId == iu.intermedioId);
                    });
                  },
                ),
              ),
