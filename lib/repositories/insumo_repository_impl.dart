...
Future<Insumo> obtenerPorCodigo(String codigo) async {
  ...
  try {
    ...
    do {
      ...
      codigo = 'I-${(count.count! + 1 + intentos).toString().padLeft(3, '0')}';
  ...
  Future<void> desactivarPorProveedor(String proveedorId) async {
    try {
      ...
      final insumos = await _db.collection(_coleccion)
  ...
  @override
  Future<List<Insumo>> filtrarInsumosPorCategorias(List<String> categorias) async {
    try {
      final querySnapshot = await _db
          .collection(_coleccion)
