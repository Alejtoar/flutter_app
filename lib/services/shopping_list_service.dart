...
class ShoppingListItem {
  ...
  @override
  String toString() => '${insumo.nombre} - ${cantidad.toStringAsFixed(2)} $unidad (Costo: \$${costoItem.toStringAsFixed(2)})';
}
...
class ShoppingListService {
  ...
  Future<GroupedShoppingListResult> generateAndGroupShoppingList(List<String> eventoIds) async {
    debugPrint("[ShoppingListService] Iniciando generación COMBINADA y AGRUPADA para ${eventoIds.length} eventos.");
    _clearCaches();
    ...
    for (final eventoId in eventoIds) {
       debugPrint("[ShoppingListService] Procesando evento combinado: $eventoId");
       try {
         await _processSingleEventIntoTotals(totals, eventoId);
       } catch (e, st) {
          debugPrint("[ShoppingListService][ERROR] Falló procesamiento $eventoId (combinado): $e\n$st");
          // Continuar con los otros eventos
       }
    }

    // 2. Convertir totales a lista de ShoppingListItem
     debugPrint("[ShoppingListService] Totales por unidad calculados: ${totals.length} insumos únicos.");
    final List<ShoppingListItem> flatList = await _mapTotalsToShoppingListItems(totals);

    // 3. Agrupar la lista plana por proveedor
    debugPrint("[ShoppingListService] Agrupando ${flatList.length} items por proveedor...");
    final groupedList = await _groupShoppingListByProvider(flatList);
    debugPrint("[ShoppingListService] Agrupación finalizada. ${groupedList.length} grupos (incluyendo sin proveedor).");

    return groupedList;
  }

  // --- Función Interna para Procesar UN Evento y añadir a Totales ---
  Future<void> _processSingleEventIntoTotals(ShoppingListTotals totals, String eventoId) async {
     ...
     for (final insumoEv in insumosEvento) {
       await _addInsumoQuantity(totals, insumoEv.insumoId, insumoEv.unidad, insumoEv.cantidad);
     }

     // 3. Procesar Intermedios Directos (recursivo)
     for (final intermedioEv in intermediosEvento) {
       await _processIntermedio(totals, intermedioEv.intermedioId, intermedioEv.cantidad.toDouble());
     }
  ...
  Future<void> _addInsumoQuantity(ShoppingListTotals totals, String insumoId, String? unidad, double quantity) async {
    if (insumoId.isEmpty || quantity <= 0) return;

    String unidadNormalizada = unidad?.trim().toLowerCase() ?? 'unidad'; // Usar 'unidad' o similar si es null/vacío
    ...
    totals[insumoId]![unidadNormalizada] = (totals[insumoId]![unidadNormalizada] ?? 0) + quantity;
  ...
  Future<void> _processPlatoEvento(ShoppingListTotals totals, PlatoEvento platoEvento) async {
    final platoBase = await _getPlatoBase(platoEvento.platoId);
    if (platoBase == null) {
       debugPrint("[ShoppingListService][WARN] Plato base ID ${platoEvento.platoId} no encontrado, omitiendo.");
       return;
    }
    if (platoBase.porcionesMinimas <= 0) {
       debugPrint("[ShoppingListService][WARN] Plato base ID ${platoBase.id} tiene porcionesMinimas <= 0, omitiendo para evitar división por cero.");
       return;
    }
    ...
    debugPrint("[ShoppingListService] Procesando PlatoEvento: ${platoBase.nombre} x ${platoEvento.cantidad} (Base: ${platoBase.porcionesMinimas} porciones)");
    ...
    debugPrint("    Ratio Plato (Rp): $cantidadPlatoEvento / ${platoBase.porcionesMinimas} = $ratioPlato");
    ...
    final List<InsumoRequerido> insumosBaseReq = await _getInsumosRequeridos(platoBase.id!);
    for (final irBase in insumosBaseReq) {
       if (!insumosRemovidos.contains(irBase.insumoId)) {
          ...
          await _addInsumoQuantity(totals, irBase.insumoId, insumoBaseDef?.unidad, cantidadNecesaria);
           debugPrint("      Insumo Req: ${insumoBaseDef?.nombre ?? irBase.insumoId} -> Cant: ${irBase.cantidad} * Rp $ratioPlato = $cantidadNecesaria");
       } else { debugPrint("      Insumo Req base ${irBase.insumoId} REMOVIDO."); }
    }
    ...
    for (final itemExtra in platoEvento.insumosExtra ?? []) {
        ...
        await _addInsumoQuantity(totals, itemExtra.id, insumoBaseDef?.unidad, cantidadNecesariaExtra);
         debugPrint("      Insumo Extra: ${insumoBaseDef?.nombre ?? itemExtra.id} -> Cant: ${itemExtra.cantidad} * Rp $ratioPlato = $cantidadNecesariaExtra");
    }

     // --- Procesar Intermedios Requeridos del Plato Base ---
     final List<IntermedioRequerido> intermediosBaseReq = await _getIntermediosRequeridos(platoBase.id!);
     for (final irBase in intermediosBaseReq) {
       if (!intermediosRemovidos.contains(irBase.intermedioId)) {
          ...
          await _processIntermedio(totals, irBase.intermedioId, cantidadIntermedioNecesaria);
           // El debugPrint del intermedio se hará dentro de _processIntermedio
       } else { debugPrint("      Intermedio Req base ${irBase.intermedioId} REMOVIDO."); }
     }
     ...
     for (final itemExtra in platoEvento.intermediosExtra ?? []) {
        final cantidadIntermedioExtraNecesaria = itemExtra.cantidad * cantidadPlatoEvento;
        await _processIntermedio(totals, itemExtra.id, cantidadIntermedioExtraNecesaria);
     }
  }

  // Procesa un Intermedio, obteniendo sus InsumosUtilizados
  Future<void> _processIntermedio(ShoppingListTotals totals, String intermedioId, double cantidadTotalNecesariaIntermedio) async {
    ...
    final intermedioBase = await _getIntermedioBase(intermedioId);
     if (intermedioBase == null) {
       debugPrint("[ShoppingListService][WARN] Intermedio base ID $intermedioId no encontrado, omitiendo.");
       return;
    }
     if (intermedioBase.cantidadEstandar <= 0) {
       debugPrint("[ShoppingListService][WARN] Intermedio base ID ${intermedioBase.id} tiene cantidadEstandar <= 0, omitiendo para evitar división por cero.");
       return;
    }

    debugPrint("    Procesando Intermedio: ${intermedioBase.nombre} - Se necesitan: $cantidadTotalNecesariaIntermedio ${intermedioBase.unidad} (Base: ${intermedioBase.cantidadEstandar} ${intermedioBase.unidad})");

    // --- Calcular Ratio del Intermedio ---
    // Este ratio se aplicará a las cantidades de los InsumosUtilizados de este intermedio.
    final double ratioIntermedio = cantidadTotalNecesariaIntermedio / intermedioBase.cantidadEstandar;
    debugPrint("      Ratio Intermedio (Ri): $cantidadTotalNecesariaIntermedio / ${intermedioBase.cantidadEstandar} = $ratioIntermedio");


    final List<InsumoUtilizado> insumosUtilizados = await _getInsumosUtilizados(intermedioId);
    for (final iu in insumosUtilizados) {
       ...
       await _addInsumoQuantity(totals, iu.insumoId, insumoBaseDef?.unidad, cantidadNecesaria);
        debugPrint("        Insumo Utilizado: ${insumoBaseDef?.nombre ?? iu.insumoId} -> Cant: ${iu.cantidad} * Ri $ratioIntermedio = $cantidadNecesaria");
  ...
  // --- Métodos para obtener datos base (con caché simple) ---
   Future<Plato?> _getPlatoBase(String platoId) async {
     // ... (igual que antes)
      ...
      try { final p = await platoRepo.obtener(platoId); _platoCache[platoId] = p; return p; }
      catch (_) { return null; }
   }
   Future<Intermedio?> _getIntermedioBase(String intermedioId) async {
      // ... (igual que antes, usando obtener(id))
       if (_intermedioCache.containsKey(intermedioId)) return _intermedioCache[intermedioId];
       try { final i = await intermedioRepo.obtener(intermedioId); _intermedioCache[intermedioId] = i; return i; }
       catch (_) { return null; }
   }
    Future<Insumo?> _getInsumoBase(String insumoId) async {
      ...
      try {
         ...
         final insumos = await insumoRepo.obtenerVarios([insumoId]); // Asume obtenerVarios existe
   ...
   Future<List<InsumoRequerido>> _getInsumosRequeridos(String platoId) async {
      // ... (igual que antes)
       if (_insumoRequeridoCache.containsKey(platoId)) return _insumoRequeridoCache[platoId]!;
       final lista = await insumoRequeridoRepo.obtenerPorPlato(platoId);
       _insumoRequeridoCache[platoId] = lista; return lista;
   }
   Future<List<IntermedioRequerido>> _getIntermediosRequeridos(String platoId) async {
       // ... (igual que antes)
       if (_intermedioRequeridoCache.containsKey(platoId)) return _intermedioRequeridoCache[platoId]!;
       final lista = await intermedioRequeridoRepo.obtenerPorPlato(platoId);
       _intermedioRequeridoCache[platoId] = lista; return lista;
   }
   Future<List<InsumoUtilizado>> _getInsumosUtilizados(String intermedioId) async {
       // ... (igual que antes)
        if (_insumoUtilizadoCache.containsKey(intermedioId)) return _insumoUtilizadoCache[intermedioId]!;
        final lista = await insumoUtilizadoRepo.obtenerPorIntermedio(intermedioId);
        _insumoUtilizadoCache[intermedioId] = lista; return lista;
   }


  // Convierte el mapa de totales por unidad a una lista plana de ShoppingListItem
  Future<List<ShoppingListItem>> _mapTotalsToShoppingListItems(ShoppingListTotals totals) async {
    ...
    final mapaInsumos = { for (var i in insumos) if (i.id != null) i.id! : i };

    totals.forEach((insumoId, unidadesMap) {
      final insumoBase = mapaInsumos[insumoId];
      if (insumoBase != null) {
        unidadesMap.forEach((unidad, cantidad) {
           if (cantidad > 0) { // Solo añadir si la cantidad es positiva
           double costoDelItem = 0;
            if (insumoBase.precioUnitario > 0) { // Solo si hay precio
                costoDelItem = cantidad * insumoBase.precioUnitario;
            } else {
                debugPrint("[ShoppingListService][WARN] Insumo ${insumoBase.nombre} no tiene precioUnitario, costo será 0.");
            }
              flatList.add(ShoppingListItem(
                insumo: insumoBase,
          ...
          debugPrint("[ShoppingListService][WARN] No se encontró Insumo ID $insumoId al mapear resultado final.");
  ...
  Future<GroupedShoppingListResult> _groupShoppingListByProvider(List<ShoppingListItem> flatList) async {
     if (flatList.isEmpty) return {};
      ...
      final mapaProveedores = { for (var p in todosProveedores) if (p.id != null) p.id! : p };

      // 2. Agrupar usando collection.groupListsBy
      final groupedByProviderId = flatList.groupListsBy((item) => item.insumo.proveedorId);
      ...
      groupedByProviderId.forEach((proveedorId, items) {
          final proveedor = (proveedorId.isNotEmpty) ? mapaProveedores[proveedorId] : null;
      ...
      final sortedKeys = finalGroupedList.keys.toList()
        ..sort((a, b) {
            if (a == null && b == null) return 0;
            if (a == null) return 1; // null (sin proveedor) al final
            if (b == null) return -1;
            return a.nombre.compareTo(b.nombre); // Ordenar por nombre de proveedor
        });
